...
![zk广播数据一致性](pic/zk广播数据一致性.png)

zk收到客户端的写请求流程：

1. Leader收到写请求，生成一个事务，包含zxid
2. Leader开始广播该事务，所有节点的通讯都是由FIFO队列维护
3. Follower接受到事务后，将事务写入本地磁盘，写入成功后返回Leader一个ACK
4. Leader收到过半的ACK后，开始提交本事务，并广播事务提交信息(非强一致性)
5. 从节点开始提交本事务

zab协议的有序性保证：

+ 服务前用TCP协议通讯，保证网络传输的有序性
+ 节点前维护了FIFO队列，保证全局有序性
+ 通过全局递增的zxid保证因果有序性
...
2. 分布式锁
   ...
   ![zk分布式锁](pic/zk分布式锁.png)

   + 系统ABC都去访问 /locks 节点，分别创建带顺序号的临时节点
   + 获取 /locks 节点下所有子节点，判断自己是否是最小的那个节点
   + 如果是，则拿到锁，如果不是，则监听比自己小1的节点变化
   + 释放锁：执行完操作后，把创建的节点给删掉

3. 统一命名服务

   ZK是通过树形结构存储数据节点的，对于每个节点的全路径，其必定是唯一的，以节点的全路径作为命名方式。

   ![zk统一命名服务](pic/zk统一命名服务.png)
...
5. 统一配置管理
   ...
   系统ABC的公共配置放在Znode中，系统ABC监听这个Znode节点是否变更

   ![zk统一配置管理](pic/zk统一配置管理.png)
